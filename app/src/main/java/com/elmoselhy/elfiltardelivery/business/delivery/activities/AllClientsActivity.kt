...
class AllClientsActivity : BaseActivity() {
    ...
    private fun getClients() {
        appViewModel.getAllClients(clientsQuery, onResult = {
            updateDataList(it)
        })
    }
    ...
    private fun setUpPageActions() {
        ...
        binding.appBar.title = getString(R.string.my_clients)
    ...
    private fun setUpList() {
        adapter = ClientAdapter(dataList) { position, model ->
            startActivity(
                Intent(this, ClientDetailsActivity::class.java).putExtra(
